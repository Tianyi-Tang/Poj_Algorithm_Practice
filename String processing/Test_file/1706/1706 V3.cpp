...
void getALlInput(){
	...
	while(getline(cin,input)){
		if(isBlank())
			...
			while(getline(cin,input)){
				if(isBlank() || allRef[referNum][n-1] == input)
					break;
				allRef[referNum][n++] = input;
			}
		}
...
void ouput(){
	int referNum,n;
	for(int i =1;i < maxReferNum && referOrder[i] != 0;i++){
		referNum = referOrder[i];
		cout << "[" << i;
		n = 1;
		while(allRef[referNum][0][n] != ']')
			n++;
		cout << allRef[referNum][0].substr(n,allRef[referNum][0].length()-n) << endl;
		n =1;
		while(allRef[referNum][n].length() != 0)
			cout << allRef[referNum][n++] << endl;
		cout << endl;
	}
}
